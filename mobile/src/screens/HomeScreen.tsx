  ...
  const primaryActions = [
    {
      ...
      onPress: () => {
        // Si estamos en las tabs, usar jumpTo para ir a la tab Shop
        // Si no, usar navigate normal
        const tabNavigator = navigation.getParent();
        if (tabNavigator?.getState()?.type === 'tab') {
          tabNavigator.navigate('Shop');
        } else {
    ...
    },
    {
      ...
      onPress: () => {
        // Si estamos en las tabs, usar jumpTo para ir a la tab MyESIMs
        const tabNavigator = navigation.getParent();
        if (tabNavigator?.getState()?.type === 'tab') {
          tabNavigator.navigate('MyESIMs');
        } else {
          navigation.navigate('MyESIMs');
        }
      },
    },
  ];
                ...
                >
                  <View style={styles.primaryActionContent}>
                    ...
                    <View style={styles.primaryActionTextContainer}>
                      ...
                      <Text 
                        style={styles.primaryActionSubtitle}
                        numberOfLines={1}
                        adjustsFontSizeToFit={true}
...
const styles = StyleSheet.create({
  ...
  primaryActionWrapper: {
    width: '47.5%', // Ancho exacto en lugar de flex
  },
  primaryActionAnimated: {
    height: 160, // Altura fija
  },
  primaryActionCard: {
    height: 160, // Altura fija igual
    borderRadius: BORDER_RADIUS.xl,
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 8 },
    shadowOpacity: 0.2,
